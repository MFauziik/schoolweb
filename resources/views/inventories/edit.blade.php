    ...
    <div
        ...
        <div class="max-w-4xl mx-auto">
            <div class="mb-8">
                ...
                <p class="text-gray-600 dark:text-gray-300">Update data fasilitas: {{ $inventory->nama_barang }}</p>
            </div>
                ...
                <form action="{{ route('inventories.update', $inventory) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="form-group">
                            ...
                            <select name="kategori" id="kategori" class="form-select">
                                ...
                                <option value="Elektronik"
                                    {{ old('kategori', $inventory->kategori) == 'Elektronik' ? 'selected' : '' }}>
                                    Elektronik</option>
                                <option value="Laboratorium"
                                    {{ old('kategori', $inventory->kategori) == 'Laboratorium' ? 'selected' : '' }}>
                                    Laboratorium</option>
                                <option value="Olahraga"
                                    {{ old('kategori', $inventory->kategori) == 'Olahraga' ? 'selected' : '' }}>Olahraga
                                </option>
                                <option value="Perabotan"
                                    {{ old('kategori', $inventory->kategori) == 'Perabotan' ? 'selected' : '' }}>
                                    Perabotan</option>
                                <option value="Buku"
                                    {{ old('kategori', $inventory->kategori) == 'Buku' ? 'selected' : '' }}>Buku
                                </option>
                                <option value="Alat Tulis"
                                    {{ old('kategori', $inventory->kategori) == 'Alat Tulis' ? 'selected' : '' }}>Alat
                                    Tulis</option>
                            </select>
                            @error('kategori')
                            <p class="form-error">{{ $message }}</p>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-select">
                                ...
                                <option value="Baik"
                                    {{ old('status', $inventory->status) == 'Baik' ? 'selected' : '' }}>Baik</option>
                                <option value="Rusak Ringan"
                                    {{ old('status', $inventory->status) == 'Rusak Ringan' ? 'selected' : '' }}>Rusak
                                    Ringan</option>
                                <option value="Rusak Berat"
                                    {{ old('status', $inventory->status) == 'Rusak Berat' ? 'selected' : '' }}>Rusak
                                    Berat</option>
                                <option value="Perlu Perbaikan"
                                    {{ old('status', $inventory->status) == 'Perlu Perbaikan' ? 'selected' : '' }}>Perlu
                                    Perbaikan</option>
                                <option value="Hilang"
                                    {{ old('status', $inventory->status) == 'Hilang' ? 'selected' : '' }}>Hilang
                                </option>
                            </select>
                            @error('status')
                            <p class="form-error">{{ $message }}</p>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="lokasi_barang" id="lokasi_barang" class="form-input"
                            ...
                            value="{{ old('lokasi_barang', $inventory->lokasi_barang) }}">
                        @error('lokasi_barang')
                        <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="is_active" id="is_active" class="form-select">
                            <option value="1" {{ old('is_active', $inventory->is_active) == 1 ? 'selected' : '' }}>Aktif
                            </option>
                            <option value="0" {{ old('is_active', $inventory->is_active) == 0 ? 'selected' : '' }}>Tidak
                                Aktif</option>
                        </select>
                        @error('is_active')
                        <p class="form-error">{{ $message }}</p>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gray-200 dark:border-gray-600">
                        ...
                        <a href="{{ route('inventories.show', $inventory) }}" class="btn-secondary text-center">
